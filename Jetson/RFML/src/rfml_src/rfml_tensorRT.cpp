...
void readFloatVec(const std::string& fileName, float f_buffer[INPUT_H*INPUT_W])
{
  uint8_t char_buffer[INPUT_H*INPUT_W*sizeof(float)];
  
  std::ifstream infile(locateFile(fileName), std::ifstream::binary);
  infile.read(reinterpret_cast<char*>(char_buffer), INPUT_H*INPUT_W*4);
  for (int i = 0; i < INPUT_H*INPUT_W; i++)
  {
    char charr[4];
    charr[0] = char_buffer[i*4];
    charr[1] = char_buffer[i*4+1];
    charr[2] = char_buffer[i*4+2];
    charr[3] = char_buffer[i*4+3];
...
int main(int argc, char** argv)
{
  ...
  readFloatVec(filename, fileData);
