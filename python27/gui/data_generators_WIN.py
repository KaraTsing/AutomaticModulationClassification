...
def gen_bpsk(snr):
    generate_waveform('bpsk', snr)
    
def gen_gfsk(snr):
    generate_waveform('gfsk', snr)
    
def gen_cpfsk(snr):
    generate_waveform('cpfsk', snr)
    
def gen_qpsk(snr):
    src = generate_waveform('qpsk', snr)
    return src
    
def gen_8psk(snr):
    src = generate_waveform('8psk', snr)
    return src
    
def gen_qam16(snr):
    generate_waveform('qam16', snr)
    
def gen_pam4(snr):
    generate_waveform('pam4', snr)

def gen_qam64(snr):
    generate_waveform('qam64', snr)
    
def gen_amssb(snr):
    generate_waveform('amssb', snr)

def gen_amdsb(snr):
    generate_waveform('am', snr)
    
def gen_wbfm(snr):
    src,mod,chan = generate_waveform('fm', snr)
    return src,mod,chan
