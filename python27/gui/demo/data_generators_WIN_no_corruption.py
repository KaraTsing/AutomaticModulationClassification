...
def gen_amdsb(channel_prop=cp_default):
    src,mod,chan = generate_waveform('amdsb', channel_prop)
    return src,mod,chan
    
def gen_wbfm(channel_prop=cp_default):
    src,mod,chan = generate_waveform('wbfm', channel_prop)
    return src,mod,chan
...
def generate_waveform(modulation_type_requested, channel_prop):
    ...
    all_keys = {"bpsk":0,"8psk":0, "qpsk":0, "pam4":0, "qam16":0, "qam64":0, "gfsk":0, "cpfsk":0, "wbfm": 1, "amdsb": 1, "amssb": 1 }
    discrete_keys = {"bpsk":0,"8psk":1, "qpsk":2, "pam4":3, "qam16":4, "qam64":5, "gfsk":6, "cpfsk":7 }
    continuous_keys = {"wbfm": 0, "amdsb": 1, "amssb": 2}    
    ...
    print "transmitter keys: " + str(transmitter_keys)
    
    alphabet_type = transmitter_keys[mod_alphabet]  
    print "Alphabet type: " + str(alphabet_type)
    ...
    print "SRC ID: "  + str(src.src.unique_id())
    mod = mod_type[mod_type_key]()
    print "Mod block: "+ str(mod.mod)
