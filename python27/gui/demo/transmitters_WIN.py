...
ebw = 0.35 # excess bandwidth (alpha for RRC filter)
...
# since it doesn't make sense to apply RRC filters to analog signals these are separate classes
class transmitter_fm(gr.hier_block2):
    ...
    def __init__(self):
        ...
        gr.io_signature(1, 1, gr.sizeof_gr_complex))
        samp_rate = 800e3
        self.rate = samp_rate/44.1e3
        #self.mod = analog.wfm_tx( audio_rate=44100.0, quad_rate=220.5e3 )
        self.mod = analog.wfm_tx( audio_rate=44100.0, quad_rate=44100.0*2 )
        self.connect( self, self.mod, self )
...
# class transmitter_am(gr.hier_block2):
    ...
    # def __init__(self):
        ...
        # self.src = analog.sig_source_c(samp_rate, analog.GR_SIN_WAVE, 0e3, 1.0)
...
class transmitter_am(gr.hier_block2):
    ...
    def __init__(self):
        ...
        self.cnv = blocks.float_to_complex()
        self.connect( self, self.interp,  self.mul, self.add, self.mod, self.cnv, self )
        self.connect( self.src, (self.mod,1) )
